...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"tree": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	message := "Welcome to the Seed JavaScript console!\n\n"

	// Print some generic Geth metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + tree.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + tree.blockNumber + " (" + new Date(1000 * tree.getBlock(tree.blockNumber).timestamp) + ")\n";
		try {
